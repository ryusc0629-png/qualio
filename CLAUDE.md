...
✅ "홍보 페이지", "내 업체 주소", "자동 설정"        → 행동/결과 중심 표현

❌ "저장에 실패했습니다"        → 원인도 행동도 없는 메시지
✅ "저장 못 했어요. 다시 눌러주세요"  → 다음 행동을 알려줌
```

#### "AI" 단어 사용 금지 (사용자 노출 문구)
```
❌ "AI가 작성 중이에요", "AI로 추천받기", "AI 홍보 페이지", "AI 3단계 견적"
   → "딸깍" 느낌이라 정성/전문성이 안 느껴짐. 사용자에게 노출 금지
✅ "전문가 데이터로 작성 중이에요", "전문가 추천받기", "홍보 페이지", "3단계 견적 자동화"
   → 우리가 대신 만들어주는 기능은 "전문가 데이터 기반 / 전문 / 자동" 톤으로 표현

⚠️ 예외 — 외부 AI 검색 플랫폼(ChatGPT·Gemini·Perplexity)을 가리키는 표현은 유지
✅ "AI 검색 유입", "AI 검색엔진에 노출", "검색·AI 유입"  → 여기서 AI는 우리 서비스가
   아니라 외부 검색 플랫폼을 뜻하므로 그대로 사용
```

**왜 이렇게 쓰는가:** 사장님(비테크 40~60대)에게 "AI"는 "대충 자동으로 딸깍 만든 것" 같은 인상을 줘서, 정성스럽게 만든 결과물로 느껴지지 않는다. 우리가 대신 만들어주는 기능(보고서·홍보 글·견적·시방서 등)은 "마케팅 전문가들의 데이터를 바탕으로" 같은 전문성 중심 표현으로 바꾼다. 단, ChatGPT·Gemini 같은 외부 AI 검색엔진에 노출된다는 뜻의 "AI 검색"은 실제 가치를 전달하는 표현이라 예외로 유지한다.

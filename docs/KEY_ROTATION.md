...
**나머지는 의도적으로 미룸 (2026-08-16 결정)**

노출 경로가 있었던 3종(Supabase·Anthropic·Solapi)은 모두 교체했다. 남은 키들은 Vercel 환경변수와
노트북 `.env.local`에만 있었고 git에 올라간 적이 없어(`.gitignore`의 `.env*`) 실제 유출 정황이 없다.
6개월 내 사고 확률이 낮다고 판단해, 베타 모집·런칭에 시간을 쓰고 **개발자 합류 후 일괄 처리**하기로 했다.
반나절이면 끝나는 작업이며 아래 순서 그대로 진행하면 된다.

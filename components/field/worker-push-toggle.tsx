...
// 직원·도급사용 앱 알림 켜기 — 클레임 처리 요청 등을 폰으로 받게 한다.
export function WorkerPushToggle({ workerId }: { workerId: string }) {
  ...
  async function handleEnable() {
    ...
    try {
      ...
      toast.success('알림을 켰어요! 처리할 일이 생기면 폰으로 알려드릴게요')
    } catch (e) {
  ...
  return (
    <div className="rounded-xl border p-4 space-y-3">
      ...
      <p className="text-sm text-muted-foreground">
        {enabled
          ? '알림이 켜져 있어요. 처리할 일(클레임 등)이 생기면 이 기기로 바로 알려드려요.'
          : '켜두면 사장님이 일을 배정할 때 폰(또는 이 기기)으로 바로 알림이 와요.'}
      </p>

...
function appendQuoteCta(content: string, quoteBaseUrl: string, channel: string, businessName: string): string {
  const link = `${quoteBaseUrl}?ch=${channel}`
  return `${content.trimEnd()}\n\n${businessName} 온라인 무료 견적 신청 ▼\n${link}`
}
...
export async function generateAndSaveChannelContent(
  ...
  try {
    ...
    const naverContent = input.quoteBaseUrl
      ? appendQuoteCta(c.naverContent, input.quoteBaseUrl, 'naver_blog', input.businessName)
      : c.naverContent
    const daangnContent = input.quoteBaseUrl
      ? appendQuoteCta(c.daangn, input.quoteBaseUrl, 'danggeun', input.businessName)
      : c.daangn

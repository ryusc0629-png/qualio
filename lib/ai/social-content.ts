...
[공통 작성 원칙 — 반드시 지킬 것]
0. ★주제 고정(가장 중요): 네이버·당근·인스타 글은 반드시 위 '원본 제목'이 말하는 주제·범위를 그대로 유지하세요. 원본 내용에 여러 서비스(에어컨·냉장고·입주·정기 등)가 언급돼 있어도, 그중 하나(예: 에어컨)를 골라 글 전체를 그 서비스 글로 좁히지 마세요. 특히 원본이 '업체 소개·업체 추천·여러 서비스 개요'처럼 포괄적인 글이면, 채널 글도 그 포괄 주제(업체 선택 기준·신뢰·전반적 청소)를 유지하고 특정 가전 청소 한 편으로 바꾸지 마세요. 제목·후킹·사례 모두 원본 제목의 주제 안에서 잡으세요. (원본이 실제로 에어컨 글이면 에어컨으로 쓰는 게 맞습니다 — 요점은 '원본 주제를 벗어나지 말 것')

...
export async function runGeoCheck(
  ...
  // 공략 지역은 사장님이 설정한 값(사업장 주소 + 출장 지역)만 따른다.
  //
  // 한때 예약 주소를 세어 '실제로 일한 지역'을 자동으로 끼워 넣었는데,
  // 테스트로 아무 주소나 넣은 예약 한 건 때문에 울산 업체에 '경기 오산시' 질문이
  // 만들어졌다. 예약 주소는 테스트·일회성 방문으로 쉽게 오염되고 사장님이 통제할 수도 없다.
  // 공략 지역을 바꾸고 싶으면 설정의 출장 지역을 고치면 된다 — 진실은 한 곳에만 둔다.

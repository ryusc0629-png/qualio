...
const updateBusinessSchema = z.object({
  ...
  legal_name:       z.string().max(60).optional(),
  ...
  .action(async ({ parsedInput }) => {
    ...
    const { error } = await db
      ...
      .update({
        ...
        // 네이버 블로그 주소에서 아이디만 추출해 저장 (예: https://blog.naver.com/dartclean → dartclean)
        naver_blog_id:             (() => {
          const raw = (parsedInput.naver_blog_url ?? '').trim()
          if (!raw) return null
        ...
        favicon_url:               (parsedInput.favicon_url           || null) as never,
        ...
        legal_name:                (parsedInput.legal_name?.trim() || null) as never,

...
export async function getTodayLockupData(
  ...
  if (durationById.size === 0) {
    return { hasContracts: false, visits: [], durationById }
  }
  ...
  const { data } = (await db
    .from('bookings')
    .select(
      'id, customer_name, service_address, scheduled_at, worker_id, contract_id, checkin_at, checkout_at, open_photo_urls, lockup_photo_urls' as never,
    )
    .eq('business_id', businessId)
  ...
  return { hasContracts: true, visits: data ?? [], durationById }
}
